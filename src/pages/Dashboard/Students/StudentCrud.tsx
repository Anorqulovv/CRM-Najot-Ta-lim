...
const StudentsCrud = () => {
  ...
  const [parentPhone2, setParentPhone2] = useState("+998")
  ...
  const { data: groupInfo = {} } = groupPathId ? GetById(groupPathId, cookies.accessToken, QueryPATH.groupsMore, "/groups") : { data: {} }
  ...
  useEffect(() => {
    if (groupPathId && groupInfo?.id) {
      setGroupId(Number(groupInfo.id))
      setDirectionId(groupInfo.directionId ? Number(groupInfo.directionId) : null)
      setTeacherId(groupInfo.teacherId ? Number(groupInfo.teacherId) : null)
      setBranchId(groupInfo.branchId ? Number(groupInfo.branchId) : null)
    }
  }, [groupPathId, groupInfo])
  ...
  useEffect(() => {
    if (moreInfo && studentId) {
      ...
      if (moreInfo.parent) {
        ...
        setParentPhone2(moreInfo.parent?.phone2 ?? "+998")
  ...
  const isBranchDisabled = !!groupPathId || isReadOnly
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
        ...
        <SectionCard title="Shaxsiy ma'lumotlar"
          ...
          <Field label={<><PhoneOutlined /> Telefon raqami</>} hint={isReadOnly ? undefined : "Format: +998901234567"} required={!isUpdate && !isReadOnly}>
            <Input value={phone} onChange={(e) => setPhone(normalizeUzPhoneInput(e.target.value))} disabled={isReadOnly} size="large" placeholder="+998901234567" style={C.inputStyle} required={!isUpdate && !isReadOnly} />
          </Field>
          {!isReadOnly && (
        ...
        <SectionCard title="O'quvchi ma'lumotlari" subtitle={isReadOnly ? "Faqat ko'rish huquqi" : "Guruh va karta ID si ixtiyoriy"}>
          ...
          <Field
            ...
            hint={
              groupPathId
                ? "Guruh filialidan avtomatik olindi"
                : isReadOnly
                  ? undefined
                  : "Ixtiyoriy"
            }
            ...
            ? (
              <div className="grid grid-cols-1 gap-5 p-5 md:grid-cols-2 md:gap-x-7 md:p-6">
                ...
                <Field label={<><PhoneOutlined /> Telefon raqami</>} hint={isReadOnly ? undefined : "Format: +998901234567"} required={!isUpdate && hasParent && !moreInfo?.parent && !isReadOnly}>
                  <Input value={parentPhone} onChange={(e) => setParentPhone(normalizeUzPhoneInput(e.target.value))} disabled={isReadOnly} size="large" placeholder="+998901234567" style={C.inputStyle} />
                </Field>
                <Field label={<><PhoneOutlined /> 2-Telefon raqami</>} hint={isReadOnly ? undefined : "Ixtiyoriy — ikkinchi aloqa raqami"}>
                  <Input value={parentPhone2} onChange={(e) => setParentPhone2(ensureUzPhonePrefix(e.target.value))} disabled={isReadOnly} size="large" placeholder="+998901234568" style={C.inputStyle} />
                </Field>

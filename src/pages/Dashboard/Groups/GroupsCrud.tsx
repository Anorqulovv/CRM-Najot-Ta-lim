...
const GroupsCrud = () => {
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    const originalStartDate = moreInfo?.startDate ? dayjs(moreInfo.startDate) : null
    const originalEndDate = moreInfo?.endDate ? dayjs(moreInfo.endDate) : null

    const formattedStartDate = startDate?.format(dateFormat) ?? null
    const formattedEndDate = endDate?.format(dateFormat) ?? null

    const startDateChanged = !isUpdate || (
      formattedStartDate !== originalStartDate?.format(dateFormat)
    )

    const endDateChanged = !isUpdate || (
      formattedEndDate !== originalEndDate?.format(dateFormat)
    )

    if (startDateChanged && startDate && startDate.isBefore(today, "day")) {
      return alert("Boshlanish sanasi bugungi kundan oldin bo'lishi mumkin emas")
    }

    if (endDateChanged && endDate && endDate.isBefore(today, "day")) {
      return alert("Tugash sanasi bugungi kundan oldin bo'lishi mumkin emas")
    }

    if (startDate && endDate && endDate.isBefore(startDate, "day")) {
      return alert("Tugash sanasi boshlanish sanasidan oldin bo'lishi mumkin emas")
    }

    if (lessonDays.includes("Yakshanba")) {
      return alert("Yakshanba kuni dars qo'yib bo'lmaydi")
    }
  ...
  const isDirectionDisabled = !!stackPathId || !!teacherPathId || !!supportPathId || isTeacher || isSupport || isReadOnly
        ...
        <div className="grid grid-cols-1 gap-5 p-5 md:grid-cols-2 md:gap-x-7 md:p-6">
          ...
          <Field label={<><CalendarOutlined /> Boshlanish sanasi</>}>
            <DatePicker
              ...
              size="large"
              style={{ borderRadius: 9, minHeight: 44, width: '100%' }}

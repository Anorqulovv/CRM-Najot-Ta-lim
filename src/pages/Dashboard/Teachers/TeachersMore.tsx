...
const TeachersMore = () => {
    ...
    const { data: moreInfo = {}, isLoading } = GetById(teacherId, cookies.accessToken, QueryPATH.teachersMore, "/teachers")
    const { mutate: DeleteTeacher, isPending } = Delete(cookies.accessToken, `/teachers/${teacherId}`, navigate, queryClient, QueryPATH.teachers)
        ...
        <div style={{ padding: '24px 24px' }}>
                    ...
                    <div>
                        {isLoading
                            ...
                            : <>
                                ...
                                <p style={{ fontSize: 12, color: '#aaa', margin: '2px 0 0' }}>Ustoz tafsilotlari</p>
                            </>
                        }
                    </div>
                </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 18 }}>
                ...
                <div style={{ background: '#fff', borderRadius: 14, border: '1px solid rgba(0,0,0,0.07)', overflow: 'hidden' }}>
                    <div style={{ padding: '20px', background: C.accentBg, borderBottom: '#f0e8df', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
                        ...
                        {isLoading
                            ...
                            : <div style={{ textAlign: 'center' }}>
                                ...
                                <Tag color="orange" style={{ marginTop: 6 }}>Ustoz</Tag>
                            </div>
                        }
                    </div>
                    <div style={{ padding: '4px 20px 8px' }}>
                        <InfoRow icon={<FieldNumberOutlined />} label="ID" value={<Tag color="orange">#{teacherId}</Tag>} />
                ...
                {/* Right: groups */}
                <div style={{ background: '#fff', borderRadius: 14, border: '1px solid rgba(0,0,0,0.07)', overflow: 'hidden' }}>
                    <Groups title="Guruhlar" teacherPropId={teacherId ? Number(teacherId) : null} basePath={`/teachers/${teacherId}`} />
                </div>
            </div>

            {/* Delete modal */}
            <Modal open={delModal} onCancel={() => setDelModal(false)} onOk={() => DeleteTeacher()}
                ...
                <p style={{ color: '#666', fontSize: 13.5, margin: '12px 0 4px', lineHeight: 1.6 }}>
                    <strong style={{ color: '#1a1a1a' }}>"{moreInfo.fullName}"</strong> o'qituvchini o'chirsangiz, u bilan bog'liq ma'lumotlar ham o'chib ketishi mumkin.
                </p>
            </Modal>
        </div>
    )

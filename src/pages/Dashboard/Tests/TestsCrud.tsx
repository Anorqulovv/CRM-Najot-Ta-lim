...
import { Button, Input, Slider, Checkbox, Tooltip, Select, DatePicker, InputNumber } from "antd";
import {
  ArrowLeftOutlined,
  SaveOutlined,
  FileTextOutlined,
...
const TestsCrud = () => {
  ...
  const { data: myTeacherInfo = {} } = GetById(
    isTeacher ? String(userInfo?.id) : undefined,
    cookies.accessToken,
  ...
  const teacherDirectionIds = [
    ...(myTeacherInfo?.directionId ? [Number(myTeacherInfo.directionId)] : []),
    ...((myTeacherInfo?.directionIds ?? []) as any[]).map(Number),
  ];

  const uniqueTeacherDirectionIds = [...new Set(teacherDirectionIds)];

  const visibleDirections = isTeacher
    ? directions.filter((d: any) => uniqueTeacherDirectionIds.includes(Number(d.id)))
    : directions;
  ...
  useEffect(() => {
    if (moreInfo && id) {
      ...
      setStatus(
        moreInfo.status === "PAUSED" || moreInfo.status === "FINISHED"
          ? "NOACTIVE"
          : moreInfo.status ?? "ACTIVE"
      );
      setMinScore(moreInfo.minScore ?? 60);
      setStartsAt(moreInfo.startsAt ? dayjs(moreInfo.startsAt) : null);
      setEndsAt(moreInfo.endsAt ? dayjs(moreInfo.endsAt) : null);
  ...
  async function handleSubmit() {
    ...
    if (!groupId && !directionId) {
      return toast.error("Umumiy test yaratib bo'lmaydi. Yo'nalish yoki guruh tanlang");
    }
    ...
    try {
      if (startsAt && endsAt && endsAt.isBefore(startsAt)) {
        return toast.error("Test tugash vaqti boshlanish vaqtidan keyin bo'lishi kerak");
      }

      if (startsAt && endsAt && durationMinutes) {
        const maxDuration = endsAt.diff(startsAt, "minute");

        if (durationMinutes > maxDuration) {
          return toast.error(`Test davomiyligi test oralig'idan katta bo'lishi mumkin emas. Maksimum: ${maxDuration} daqiqa`);
        }
      }
      ...
      toast.error(Array.isArray(msg) ? msg[0] : msg || err?.message || "Xatolik yuz berdi");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="tests-crud-page min-h-full bg-transparent p-4 sm:p-5 lg:p-6">
      {/* Top bar */}
      ...
      {/* Test info card */}
      <div className="mx-auto mb-7 max-w-3xl rounded-3xl border border-[#f0e8de] bg-white p-5 shadow-sm sm:p-8">
        ...
        <div style={{ marginBottom: 20 }}>
          <label style={{ display: "block", fontWeight: 600, color: "#4a3728", marginBottom: 8, fontSize: 14 }}>
            Test boshlanish va tugash vaqti
          </label>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <DatePicker
              ...
              size="large"
              style={{ width: "100%" }}
              disabledDate={(current) => current ? current.isBefore(dayjs().startOf("day"), "day") : false}
            />
            <DatePicker
              showTime
              value={endsAt}
          ...
          <p style={{ margin: "6px 0 0", fontSize: 11, color: "#9c8572" }}>
            Masalan: 14:00 da boshlanib 16:00 da tugaydigan test.
          </p>
        </div>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <InputNumber
            ...
            onChange={(value) => setDurationMinutes(value === null ? null : Number(value))}
            min={1}
            max={startsAt && endsAt ? Math.max(1, endsAt.diff(startsAt, "minute")) : undefined}
            size="large"
          ...
          />
          <p style={{ margin: "6px 0 0", fontSize: 11, color: "#9c8572" }}>
            Bo‘sh qoldirilsa cheklanmagan bo‘ladi. Agar boshlanish/tugash vaqti berilgan bo‘lsa, davomiylik shu oraliqdan katta bo‘lmasligi kerak.
          </p>
        </div>
        ...
        <div className="mb-5 grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            ...
            <Select
              ...
              options={visibleDirections.map((d: any) => ({ value: d.id, label: d.name }))}
            />
            <p style={{ margin: "6px 0 0", fontSize: 11, color: "#9c8572" }}>
              {isTeacher
                ? "Faqat o'zingizga biriktirilgan yo'nalishlar ko'rinadi"
      ...
      {/* AI generate block */}
      {!id && (
        <div className="mx-auto mb-6 max-w-3xl rounded-3xl border border-[#f0e8de] bg-white p-5 shadow-sm sm:p-6">
          ...
          {aiEnabled && (
            <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-[1fr_160px_160px]">
              <div>
                <label style={{ display: "block", fontWeight: 600, color: "#4a3728", marginBottom: 8, fontSize: 14 }}>
                  Mavzu
                </label>

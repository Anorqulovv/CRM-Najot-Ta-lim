...
const SectionCard = ({ title, children }: { title: string, children: React.ReactNode }) => (
  <div className="student-more-section-card" style={{ background: '#fff', borderRadius: 14, border: '1px solid rgba(0,0,0,0.07)', overflow: 'hidden' }}>
...
const StudentMore = () => {
  ...
  const handleDeleteStudent = async () => {
    ...
    try {
      await instance(cookies.accessToken).delete(`/students/${studentId}`)

      setDelModal(false)

      await queryClient.invalidateQueries({ queryKey: [QueryPATH.students] })
      await queryClient.invalidateQueries({ queryKey: [QueryPATH.studentsMore] })
      ...
      toast.error(err?.response?.data?.message ?? "O'quvchini o'chirishda xatolik")
    } finally {
  ...
  return (
    <div className="p-4 sm:p-5 lg:p-6">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            {isLoading
              ...
              : <>
                <h2 style={{ fontSize: 19, fontWeight: 800, color: '#ffffff', margin: 0, lineHeight: 1.25, textShadow: '0 2px 10px rgba(0,0,0,0.55)' }}>{moreInfo.user?.fullName ?? "O'quvchi"}</h2>
                <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.76)', margin: '2px 0 0', fontWeight: 600, textShadow: '0 2px 8px rgba(0,0,0,0.45)' }}>O'quvchi tafsilotlari</p>
              </>
            }
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-[360px_1fr]">
        {/* Left: profile card */}
        <div className="student-more-left-card" style={{ background: '#fff', borderRadius: 14, border: '1px solid rgba(0,0,0,0.07)', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '4px 20px 8px' }}>
            ...
            <InfoRow icon={<CalendarOutlined />} label="Yaratilingan" value={formatDateTime(moreInfo.createdAt)} isLoading={isLoading} />
            <InfoRow icon={<HistoryOutlined />} label="O'zgartirilgan" value={formatDateTime(moreInfo.updatedAt)} isLoading={isLoading} />
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          <SectionCard title="Guruh ma'lumotlari">
            ...
            <InfoRow icon={<UserOutlined />} label="Ustoz" value={moreInfo.group?.teacher?.fullName} isLoading={isLoading} />
            <InfoRow icon={<ApartmentOutlined />} label="Yo'nalish" value={moreInfo.group?.direction?.name} isLoading={isLoading} />
            <InfoRow icon={<CalendarOutlined />} label="Boshlanish" value={formatDateTime(moreInfo.group?.startDate)} isLoading={isLoading} />
            <InfoRow icon={<HistoryOutlined />} label="Tugash" value={formatDateTime(moreInfo.group?.endDate)} isLoading={isLoading} />
          </SectionCard>
      ...
      {/* Delete modal */}
      {!isReadOnly && (
        <Modal open={delModal} onCancel={() => setDelModal(false)} onOk={handleDeleteStudent}

...
import {
  Button, Modal, InputNumber, Select, Table, Tag, Collapse, Progress, Spin
} from "antd";
import {
...
const TestsMore = () => {
  ...
  const canDeleteTest =
    ...
    (userRole === "TEACHER" && Number(test?.createdById) === Number(userInfo?.id));
  ...
  const resultsColumns = [
    ...
    ...(isEditable ? [{
      ...
      render: (_: any, row: any) => (
        <Button size="small" onClick={() => openReview(row.studentId)}>
          Ko'rish
        </Button>
      ),
    }, {
  ...
  const openReview = async (studentId: number) => {
    ...
    try {
      const res = await instance(cookies.accessToken).get(`/tests/${testId}/student/${studentId}/review`);
      setReviewData(res.data?.data);
    } catch (err: any) {
      toast.error(err?.response?.data?.message ?? "Xatolarni yuklashda muammo");
  ...
  return (
    <div style={{ padding: "28px", background: "transparent", minHeight: "100%" }} className="test-more-page">
      ...
      <Modal
        ...
        footer={null}
        width={900}
        title={reviewData ? `${reviewData.studentName} — ${reviewData.testTitle}` : "Xato savollar"}
      >
        {reviewLoading ? (
          <Spin />
        ) : reviewData?.attempts?.length ? (
          <Collapse accordion>
            {reviewData.attempts.map((attempt: any) => (
              <Panel
                key={attempt.resultId}
                header={`${attempt.attempt}-urinish — ${attempt.score}% — xato: ${attempt.wrongQuestions?.length ?? 0} ta`}
              >
                {(attempt.questions ?? []).map((q: any, idx: number) => (
                  ...
                  >
                    ...
                    <div style={{ marginTop: 6 }}>
                      Tanlangan javob: <b>{q.selectedChoiceText ?? "Javob berilmagan"}</b>
                    </div>
                    <div>
                      To'g'ri javob: <b style={{ color: "#16a34a" }}>{q.correctChoiceText ?? "—"}</b>
                    </div>
                    <Tag color={q.isCorrect ? "green" : "red"} style={{ marginTop: 8 }}>
                      {q.isCorrect ? "To'g'ri" : "Xato"}
                    </Tag>
                  </div>
                ))}
              </Panel>
